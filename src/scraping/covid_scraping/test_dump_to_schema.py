...
def check_values(idx, obj):
    ...
    '''
    assert isinstance(
        obj['topic'], str), "'topic' in the %d example is not a string" % idx
    assert isinstance(
        obj['sourceUrl'], str), "'source' in the %d example is not a string" % idx
    assert isinstance(
        obj['sourceName'], str), "'sourceName' in the %d example is not a string" % idx
    assert isinstance(
        obj['dateScraped'], float), "'dateScraped' in the %d example is not a float" % idx
    ...
    assert isinstance(
        obj['needUpdate'], bool), "'needUpdate' in the %d example is not a boolean" % idx
    assert isinstance(
        obj['containsURLs'], bool), "'containsURLs' in the %d example is not a boolean" % idx
    assert isinstance(
        obj['typeOfInfo'], str), "'typeOfInfo' in the %d example is not a string" % idx
    assert isinstance(
        obj['isAnnotated'], bool), "'isAnnotated' in the %d example is not a boolean" % idx
    assert isinstance(obj['responseAuthority'],
                      str), "'responseAuthority' in the %d example is not a string" % idx
    assert isinstance(
        obj['questionUUID'], str), "'questionUUID' in the %d example is not a str" % idx
    assert isinstance(uuid.UUID(
        obj['questionUUID']), uuid.UUID), "'questionUUID' in the %d example cannot be converted to UUID" % idx
    assert isinstance(
        obj['answerUUID'], str), "'answerUUID' in the %d example is not a str" % idx
    assert isinstance(uuid.UUID(
        obj['answerUUID']), uuid.UUID), "'answerUUID' in the %d example cannot be converted to UUID" % idx
    assert isinstance(
        obj['exampleUUID'], str), "'exampleUUID' in the %d example is not a str" % idx
    assert isinstance(uuid.UUID(
        obj['exampleUUID']), uuid.UUID), "'exampleUUID' in the %d example cannot be converted to UUID" % idx
    # "'questionText' in the %d example is not a string" % idx
    assert isinstance(obj['questionText'], str), pdb.set_trace()
    assert isinstance(
        obj['answerText'], str), "'answerText' in the %d example is not a string" % idx
    assert isinstance(
        obj['hasAnswer'], bool), "'hasAnswer' in the %d example is not a boolean" % idx
    assert isinstance(obj['targetEducationLevel'],
                      str), "'targetEducationLevel' in the %d example is not a string" % idx
    assert isinstance(
        obj['topic'], str), "'topic' in the %d example is not a string" % idx
    assert isinstance(
        obj['extraData'], dict), "'extraData' in the %d example is not a dict" % idx
