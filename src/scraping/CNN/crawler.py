"""
This script crawls https://www.cnn.com/interactive/2020/health/coronavirus-questions-answers/
"""
...
class Crawler():
    ...
    def get_date(self, url):
        ...
        sourcedate = soup.find('div', class_='cnnix-timestamp').get_text().split()[1:]
    ...
    def scrape(self):
        ...
        for block in blocks:
            ...
            if answer.find('a'):
                links = [(link.text, link.get('href')) for link in answer.find_all('a')]
    ...
    def dump(self, schema):
        with open(os.path.join(self.datadir, self.filename), 'w') as writer:
            for question, answer, extra_data in zip(schema.question, schema.answer, schema.extra_data):
                topic = {'topic': 'Top Coronavirus Questions', 'sourceUrl': self.url}
                contain_url = question in extra_data
                Schema(topic, self.sourcedate, contain_url, self.response_auth, question, answer, extra_data)
