    ...
    def parse(self, response):
        ...
        for q in response.css(QUESTIONS_SELECTOR):
            ...
            for selector in response.xpath(
                    "//div/h3[contains(text(), '" + question + "')]/following-sibling::*"):
                if "h3" in selector.get():
                    break
                else:
                    answer += " ".join(selector.css(ANSWER_SELECTOR).getall()
                                       ).strip() + "\n"
                    answer_html += " ".join(
                        selector.css(ANSWER_HTML_SELECTOR).getall()).strip()
        ...
        columns["name"] = [
            ...
            columns["question"])
        columns["source"] = [
            "Berufsverband der Frauenärzte (BvF)"] * len(columns["question"])
