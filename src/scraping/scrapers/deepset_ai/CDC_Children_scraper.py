...
class CovidScraper(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        categoryPaths = response.xpath(
            '//div[@class="syndicate"]/div[@class="row "]')
        for catPath in categoryPaths:
            categoryName = catPath.xpath('.//h2/text()').getall()
            if len(categoryName) == 0:
                continue
            categoryName = categoryName[0]
            qnaPaths = catPath.xpath(
                './/div[@role="tablist"]//div[@class="card"]')
            for qnaPath in qnaPaths:
                question = qnaPath.xpath(
                    './/span[@role="heading"]/text()').get()
        ...
        columns["link"] = ["https://www.cdc.gov/coronavirus/2019-ncov/prepare/children-faq.html"] * len(
            columns["question"])
        columns["name"] = [
            "Coronavirus Disease-2019 (COVID-19) and Children"] * len(columns["question"])
        columns["source"] = [
