...
class CovidScraper(scrapy.Spider):
    ...
    start_urls = [
        "https://www.salute.gov.it/portale/nuovocoronavirus/dettaglioFaqNuovoCoronavirus.jsp?id=228"]
    ...
    def parse(self, response):
        ...
        for x in response.xpath('//dl'):
            ...
            question_list = [q.strip() for q in x.xpath(
                './dt/strong[2]/text()').extract()]
            # answer is in dd
            answer_html_list = []
            answer_list = []
            for a in x.xpath('./dd')[:-1]:
                answer_html_list.append(
                    ' '.join([h.strip() for h in a.xpath('./descendant-or-self::*').extract()]))
                answer_list.append(' '.join([t.strip() for t in a.xpath(
                    './descendant-or-self::*/text()').extract()]))
            if len(question_list) == len(answer_list):
                for question_text, answer_text, answer_html in zip(
                        question_list, answer_list, answer_html_list):
        ...
        columns["link"] = [
            ...
            columns["question"])
        columns["name"] = ["FAQ - Covid-19, domande e risposte"] * \
            len(columns["question"])
        columns["source"] = ["Ministero della Salute, IT"] * \
            len(columns["question"])
